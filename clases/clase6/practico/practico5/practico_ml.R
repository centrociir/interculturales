...
pacman::p_load(
  ...
  tidytext, topicmodels, readxl, broom, ggplot2, stringr, tidyverse
)
...
boric_noticia <- read_excel("bbdd/boric-noticia.xlsx")
boric_noticia |> select(noticia)

library(tidytext)

# Suponiendo que tienes un dataframe llamado `noticias`
noticias <- tibble(id = 1, noticia = "Boric aclara su rol en la fallida compraventa de una propiedad que involucraba a un ex asesor cercano al gobierno.")

# Tokenización
tokens <- noticias %>%
  unnest_tokens(palabra, noticia)

tokens

# Frases que contienen "boric"
contexto_boric <- noticias %>%
  filter(str_detect(str_to_lower(noticia), "boric"))
...
noticias_temas %>% 
  mutate(topic = case_when(
          topic == 1 ~ "cambio climático y medio ambiente",
          topic == 2 ~ "educación",
          topic == 3 ~ "tecnología",
          topic == 4 ~ "covid/salud",
          topic == 5 ~ "economía")) %>%
